...
$location = ($_GET['location']) ? $_GET['location'] : 'N/A';
$weeksOnPerYear = intval($_GET['weeksOnPerYear']) ? $_GET['weeksOnPerYear'] : NULL;
$daysOnPerWeek = intval($_GET['daysOnPerWeek']) ? $_GET['daysOnPerWeek'] : NULL;
$onPerDay = floatval($_GET['onPerDay']) ? $_GET['onPerDay'] : floatval(0);
...
class getProfile extends Services_AMEE_ProfileItem
{
	...
	public function getComputerProfile($sProfileCategory, $productType, $brand, $modelName, $modelNumber, $category, $onStandby,$role,$location, $quantity)
	{
		try {
			...
			$aDeviceValues = array(
				...
				"role" => $role,
				"location" => $location,
				"weeksOnPerYear" => ' Energy Star figures for the selected appliance and category',
				"daysOnPerWeek" => '',
				"onPerDay" => '',
				"offPerDay" => '',
				"standbyPerDay" => '',
	...
	// GET DEVICE PROFILE
	public function getDeviceProfile($sProfileCategory, $device, $rating, $onStandby, $role,$location, $weeksOnPerYear,$daysOnPerWeek, $onPerDay, $offPerDay, $standbyPerDay, $quantity, $onFactor, $offFactor, $onStanbyFactor)
	{
		try {
			// Prepare the category for the new AMEE API Profile Item
			$sProfileCategory = $sProfileCategory;
			...
			if ($aProfileCategory['device'] == 'Personal Computers' || $aProfileCategory['device'] == 'Printer' || $aProfileCategory['device'] == 'Monitor') {
			    ...
			    if ($aProfileCategory['rating'] == 'Desktop' || $aProfileCategory['rating'] == 'Desktop no monitor') {
			        ...
			        $kWhPerHour = floatval($kWhPerDay / 5.5);
			    }
                if ($onPerDay > 0) {
			    ...
			    $kWhPerYear = getProfile::getDataItemInfo($aItemValueDefinition, 'kWhPerYear');
			}
			
			if ($weeksOnPerYear == 0 && $daysOnPerWeek == 0) {
			    $weeksOnPerYear = 'stock-averaged energy consumed, scaled from MTP figures.';
			} else if ($weeksOnPerYear == 0) {
			    $weeksOnPerYear = 0;
			    $daysOnPerWeek = 0;
			}
...
if ($device == '') { // if it is a computer
	getProfile::getComputerProfile($sProfileCategory, $productType, $brand, $modelName, $modelNumber, $category, $onStandby, $role,$location, $quantity);
} else { // if it is a computing equipment
	getProfile::getDeviceProfile($sProfileCategory, $device, $rating, $onStandby, $role,$location, $weeksOnPerYear, $daysOnPerWeek, $onPerDay, $offPerDay, $standbyPerDay, $quantity, $onFactor, $offFactor, $onStanbyFactor);
}
?>